	...
	<div class="container">
		...
		<div class="profile">
			...
			<div class="profile-image">
				...
				<img width="150" src="https://image.flaticon.com/icons/svg/2922/2922510.svg">
				
			</div>

			<div class="profile-user-settings">

				<h1 class="profile-user-name"><%=connect.getNicknameUser()%></h1>
				<br>
				<a href="view"><button class="btn-success">Timeline</button></a>
				<a href="/profil.jsp"><button class="btn-primary">Profil</button></a>
				<a href="/post.jsp"><button class="btn-secondary">Publier un post</button></a>
				<a href="/deconnexion"><button class="btn-danger">Déconnexion</button></a>
			...
			<div class="profile-stats">
				...
				<ul>
					<li><span class="profile-stat-count"></span> <a href="/nbpost"><%=infos.getNbPost() %> posts</a></li>
					<li><span class="profile-stat-count"></span><a href = "/listfollow"><%=infos.getNbFollower() %> followers</a></li>
					<li><span class="profile-stat-count"></span><a href = "/listfriend""><%=infos.getNbFollow() %> followings</a></li>
				</ul>

			</div>
		<form method="get" action="/searchuser">
			<div class="input-group-append">
				<input name="nomUser" type="text" class="form-control" placeholder="Rechercher un utilisateur" aria-label="Recipient's username" aria-describedby="basic-addon2">
				<button value="submit" class="input-group-text btn-primary">Rechercher</button>  
			</div>
		</form>		
		...
		<c:forEach items="${listeUsers}" var="itemUser">
    		<div class="card" id="demo" class="collapse">
				<div class="card-body">
					<h5 class="card-title">${itemUser}</h5>
					<a href="/addFriend/ + ${itemUser}"  class="btn btn-primary">Follow</a>
				</div>
			</div>
		</c:forEach>
​
	...
	<!-- End of container -->
		...
		<c:forEach items="${listePosts}" var="post">
		...
		<section class="cid-rYSBWaumP1" id="image3-6">
		    ...
		    <figure class="mbr-figure container">
		            ...
		            <div class="image-block" style="width: 50%;">
		                ...
		                <img src="${post.value[0]}" width="1400" alt="">
		                
		                <div class='card'>
						   <p style="padding-top : 10px;"><i> ${post.value[2]}</i> </p>	 
		            	</div>
		            	
		            	<div class='card'>
						   <p style="padding-top : 10px; color: #2C75FF"> <b>Total de likes : ${post.value[4]} </b></p>	 
		            	</div> 
			            
			            <div class="input-group mb-3">
			            <form method="get" action="/commentpost/${post.key}">
						  <div class="input-group-append">

						<c:if test="${post.value[5] ne 'true'}">
						    <a style="width: 30%; margin:auto;" href="/likePost/${post.key}"><img style="width: 30%; margin:auto;" src="https://image.flaticon.com/icons/png/512/1077/1077086.png"/></a>  
						  </c:if>
						  <c:if test="${post.value[5] eq 'true'}">
						    <a style="width: 30%; margin:auto;" href="/likePost/${post.key}"><img style="width: 30%; margin:auto;" src="https://image.flaticon.com/icons/png/512/535/535183.png"/></a>  
						  </c:if>
						  

						  	<input name="messageComment" type="text" class="form-control" placeholder="Écrire un commentaire" aria-label="Recipient's username" aria-describedby="basic-addon2">
						    <button value="submit" class="input-group-text btn-primary">Envoyer</button>  
						  ...
						  <c:forEach items="${listeCommentC}" var="comment">
						  <c:set var="i" value="${post.key}" />
						  <c:set var="j" value="${comment.value[0]}" />
						  <c:if test="${i eq j}">
						  
						    <div class="media">
							  <div class="media-body">
							    <h5 class="mt-0 mb-1">${comment.value[1]}</h5>
							   <p>${comment.value[2]}</p>
							  </div>
							</div>	
							</c:if>
